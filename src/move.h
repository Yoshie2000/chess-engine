...
public:
    MoveGen(Board* board, Move _ttMove, Move _killers[2]) : board(board), ttMove(_ttMove), onlyCaptures(false), killers{ _killers[0], _killers[1] }, moveList{ MOVE_NONE }, generatedMoves(0), returnedMoves(0), generationStage(GEN_STAGE_TTMOVE) {
        std::fill(moveList, moveList + MAX_MOVES, MOVE_NONE);
    }
    MoveGen(Board* board, Move _ttMove, bool onlyCaptures) : board(board), ttMove(_ttMove), onlyCaptures(onlyCaptures), killers{ MOVE_NONE, MOVE_NONE }, moveList{ MOVE_NONE }, generatedMoves(0), returnedMoves(0), generationStage(GEN_STAGE_TTMOVE) {
        std::fill(moveList, moveList + MAX_MOVES, MOVE_NONE);
    }
