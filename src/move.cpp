...
void generateCastling(Board* board, Move** moves, int* counter) {
    ...
    case COLOR_WHITE:
        // Kingside
        if ((board->stack->castling & 0x1) && !(occupied & 0x60)) {
            *(*moves)++ = createMove(king, king + 2) | MOVE_CASTLING;
            (*counter)++;
        }
        // Queenside
        if ((board->stack->castling & 0x2) && !(occupied & 0x0E)) {
            *(*moves)++ = createMove(king, king - 2) | MOVE_CASTLING;
            (*counter)++;
        }
        break;
    case COLOR_BLACK:
        // Kingside
        if ((board->stack->castling & 0x4) && !(occupied & C64(0x6000000000000000))) {
            *(*moves)++ = createMove(king, king + 2) | MOVE_CASTLING;
            (*counter)++;
        }
        // Queenside
        if ((board->stack->castling & 0x8) && !(occupied & C64(0x0E00000000000000))) {
            *(*moves)++ = createMove(king, king - 2) | MOVE_CASTLING;
            (*counter)++;
        }
        break;
